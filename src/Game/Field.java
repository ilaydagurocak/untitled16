...
public class Field {
    ...
    public void destroyFullRows() {
        for (int row = 0; row < this.field.length; row++) {
            ...
            if (isFullRow) {
                this.shiftRows(row);
                this.score += 10; // increment score when a row is destroyed
            }
        }
    }
    public void render(Graphics g) {
        ...
        // Draw the score on top of the field
        g.setColor(Color.WHITE);
        g.setFont(new Font("TimesRoman", Font.PLAIN, 20)); // Set font size to 20
        g.drawString("Score: " + score, 10, 50); // Draw the score at a visible position
    }
